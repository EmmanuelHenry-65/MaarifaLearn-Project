  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className={`text-sm ${mutedColor}`}>Enter your credentials to access your account.</p>
      </div>

      <form className="space-y-4" onSubmit={handleSubmit}>
        <AuthInput label="Email" name="email" type="email" placeholder="you@example.com" required />
        <AuthInput label="Password" name="password" type="password" placeholder="••••••••" required />

        {error && <p className="text-red-400 text-xs font-medium">{error}</p>}

        <div className="flex items-center justify-between text-xs">
          <label className="flex items-center gap-2 text-gray-400 cursor-pointer">
            <input type="checkbox" className="rounded bg-[rgba(17,24,50,0.6)] border-[rgba(56,78,135,0.3)]" />
            Remember me
          </label>
          <Link to="/forgot-password" className="text-cyan-400 hover:text-cyan-300 font-semibold">Forgot Password?</Link>
        </div>

        <AuthButton type="submit" disabled={submitting}>{submitting ? 'Logging in...' : 'Login'}</AuthButton>
      ...
      <p className={`text-center text-sm ${mutedColor}`}>
        Don't have an account? <Link to="/signup" className="text-cyan-400 hover:text-cyan-300 font-semibold">Create Account</Link>
      </p>
    </div>
  );
}
