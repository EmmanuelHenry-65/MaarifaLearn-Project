...
export default function ReviewPanel({ paper, answers, onBackToResults, onGenerateSimilar }: ReviewPanelProps) {
  const { theme } = useTheme();
  const [activeIndex, setActiveIndex] = useState(0);
  const question = paper.questions[activeIndex];
    ...
    <div className="flex gap-4 flex-1 min-h-0">
      ...
      <main className="flex-1 overflow-y-auto pr-1 space-y-4">
        <div className="glass-card p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className={`text-xl font-extrabold mt-1 ${textColor}`}>{question.prompt}</h2>
              <p className={`text-xs mt-1 ${mutedColor}`}>{question.topic} • {question.difficulty}</p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <InfoCard title="Correct Answer" value={question.correctAnswer} />
        </div>
        <div className={`rounded-2xl border p-5 ${panelBg}`}>
          <h3 className={`font-bold text-base ${textColor}`}>Marking Scheme</h3>
          <p className={`text-sm mt-2 ${mutedColor}`}>{question.markingScheme}</p>
        </div>
        <div className={`rounded-2xl border p-5 ${panelBg}`}>
          <h3 className={`font-bold text-base ${textColor}`}>AI Explanation</h3>
          <p className={`text-sm mt-2 ${mutedColor}`}>{question.aiExplanation}</p>
...
function InfoCard({ title, value }: { title: string; value: string }) {
  ...
  return (
    <div className="glass-card p-5">
      ...
      <p className={`text-sm mt-2 ${title === 'Your Answer' ? mutedColor : textColor}`}>{value}</p>
    </div>
  );
}