import type { ExamPaper, ExamSubject } from '../../data/examData';
...
export default function ExamDashboard({ currentSubject, recentExams, recommendedPapers, onContinue, onSelectRecommended }: ExamDashboardProps) {
  ...
  const averageMarks = Math.round(recentExams.reduce((sum, paper) => sum + paper.averageScore, 0) / Math.max(1, recentExams.length));
        ...
        <div className="glass-card p-5 col-span-3">
          ...
          <div className={`rounded-xl border p-4 ${itemBg}`}>
            ...
            <div className="mt-4 h-2 rounded-full bg-slate-200/70 overflow-hidden">
              <div className="h-full bg-gradient-to-r from-cyan-500 to-blue-500 rounded-full" style={{ width: `${continuePaper.readiness}%` }} />
            </div>
          </div>
        </div>

        <div className="glass-card p-5 col-span-2">
          <h3 className={`font-bold text-base mb-3 ${textColor}`}>Performance Summary</h3>
            ...
            {examAnalytics.topicMastery.map((topic) => (
              <div key={topic.topic}>
                ...
                <div className="h-1.5 rounded-full bg-slate-200/70 overflow-hidden">
                  <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-400" style={{ width: `${topic.value}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {recommendedPapers.map((paper) => (
            <button key={paper.id} onClick={() => onSelectRecommended(paper)} className={`text-left rounded-xl border p-3 hover:border-cyan-500/30 transition-all ${itemBg}`}>
              ...
              <p className={`text-xs mt-1 ${mutedColor}`}>{paper.type} • {paper.difficulty}</p>
              <p className="text-cyan-600 text-xs font-bold mt-2">Readiness {paper.readiness}%</p>
            </button>
          ))}
        </div>
      </div>
